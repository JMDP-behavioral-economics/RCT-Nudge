...
```{r density-response-by-intention, fig.cap = "Density of Response Speed by Intention (StatusQuo Group). \\newline \\emph{Note}: We excluded those who skipped CT."}
rct$data %>%
  filter(treat == "StatusQuo") %>%
  filter(reply == 1 & !is.na(positive) & skip_test == 0) %>%
  mutate(positive = factor(positive, levels = c(0, 1), labels = c("Negative", "Positive"))) %>%
  ggplot(aes(x = days_reply, color = factor(positive))) +
    geom_density() +
    labs(x = "Days after the HLA match letter being sent", y = "Density", color = "Intention") +
