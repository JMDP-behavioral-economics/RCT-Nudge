...
with(rawdt$data, table(days_reply, coordinate, useNA = "always"))

respond <- subset(rct$data, positive == 1)

add_vars <- c(
  "days_reply", "male", "age_demean", "I(age_demean^2)", "coordinate",
  "holidays", "hospital_per_area", "PB_per_area", "BM_per_area", "treat",
  "factor(month)", "factor(week)"
)

mods <- list(
  reformulate(add_vars, "I(test * 100)"),
  reformulate(add_vars, "I(exg_stop_test * 100)")
)
...
lm_robust(
  days_reply ~ treat + male + age_demean + I(age_demean^2) +
    coordinate + holidays + hospital_per_area + PB_per_area + BM_per_area,
  data = subset(rct$data, positive == 1 & days_reply > 0),
  se_type = "stata"
)

rct$data %>%
  # filter(reply == 1) %>%
  mutate(short_reply = case_when(
    reply == 0 ~ 0,
    days_reply <= 7 ~ positive * 100,
    TRUE ~ 0
  )) %>%
  # filter(reply == 0 | days_reply > 0) %>%
  lm_robust(
    short_reply ~ treat + male + age_demean + I(age_demean^2) +
      coordinate + holidays + hospital_per_area + PB_per_area + BM_per_area +
      factor(month) + factor(week),
    data = .,
    se_type = "stata"
...
```{r int-lm-interaction, include=FALSE}
lm_int <- rct$
...
```{r lm-positive-interaction-init, include=FALSE}
...
```{r lm-positive-interaction-init-reg}
...
```{r lm-positive-interaction-init-lh}
lm_int_positive_init$
  kable_lh(
    "Positive Intention",
...
```{r decompose-candidate, eval=FALSE}
rct$
  decompose_effect()$
