...
```{r lm-interaction-gender-coordination-age, fig.width = 15, fig.height = 10, fig.cap = "Heterogeneous Treatment Effects on CT, Donor Selection, and Donation by Sex, Age, and Experience Group", fig.pos = "H"}
rct3 <- rct$clone()
...
```{r average-characteristics-among-CT}
rct$data %>%
  filter(test == 1) %>%
  datasummary(
    (`Male` = male) +
      (`Age` = age) +
      (`First-time coordination` = coordinate1) +
      (`Skipped CT` = skip_test) +
      (`Response days` = days_reply) +
      (`Number of holidays` = holidays) +
      (`Number of hospitals` = hospital_per_area) +
      (`Number of hospitals with PBSC collection` = PB_per_area) +
      (`Number of hospitals with BM collection` = BM_per_area) ~ treat *(mean + sd),
    title = "Average Characteristics among Potential Donors Reaching CT",
    data = .,
    fmt = 2,
    align = "lcccccccc"
  ) %>%
...
```{r relative-likelihood-complier-characteristics}
data_list <- list(
  full = rct$data,
  male = subset(rct$data, male == 1),
  age20 = subset(rct$data, age < 30),
  age30 = subset(rct$data, 30 <= age & age < 40),
  age40 = subset(rct$data, 40 <= age),
  coordinate1 = subset(rct$data, coordinate1 == 1)
)

regs <- data_list %>%
  map(~ lm_robust(test ~ treat, data = .)) %>%
  map(tidy) %>%
  map(~ filter(., term != "(Intercept)")) %>%
  map(~ select(., term, estimate))

for (i in names(regs)) {
  colnames(regs[[i]]) <- c("term", i)
}
...
ratio <- regs %>%
  ...
  mutate(
    ...
    age30 = age30 / full,
    age40 = age40 / full,
    coordinate1 = coordinate1 / full
  ) %>%
  select(-full) %>%
  pivot_longer(cols = -term, names_to = "group", values_to = "ratio") %>%
  pivot_wider(names_from = term, values_from = ratio) %>%
...
ratio %>%
  knitr::kable(
    col.names = c("Characteristics", "MatchMessage", "CoordMessage", "BothMessage"),
    caption = "Relative Likelihood of Complier Characteristics",
    digits = 2,
    booktabs = TRUE,
    linesep = "",
    escape = FALSE
