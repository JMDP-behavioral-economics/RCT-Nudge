...
schedule$kable(
  ...
  notes = paste(
    "\\\\emph{Note}: See Table 1 in the main manuscript for a detailed description of the intervention of each experimental group. The control group is the StatusQuo group. The experiment was not conducted during the week beginning December 27, 2021, and ending January 3, 2022, because JMDP was closed for the New Year's holiday."
  ),
  hold = TRUE,
  font_size = 8
)
...
add_tab <- data.frame(rbind(
  c(
    ...
    ctrl_avg$skip_CT, ctrl_avg$skip_CT, ctrl_avg$skip_CT
  ),
  c(
    "Covariates",
    "", "X", "X",
...
add_tab <- data.frame(
  rbind(c("Month FE", "", "", "", "", "X", "X"))
)

attr(add_tab, "position") <- 19
...
fit_mods %>%
  ...
  kableExtra::kable_styling(font_size = 8, latex_options = "HOLD_position") %>%
  kableExtra::add_header_above(c(" " = 1, "Donor selection" = 6)) %>%
  kableExtra::footnote(
    general_title = "",
