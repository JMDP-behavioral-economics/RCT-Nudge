...
rct$data %>%
  mutate(
    attr = if_else(positive == 1 & test == 0, 100, 0),
    exg_attr = if_else(attr == 100 & exg_stop_test == 1, 100, 0),
    end_attr = if_else(attr == 100 & exg_stop_test == 0, 100, 0)
  ) %>%
  lh_robust(
    I(test * 100) ~ treat * male + age_demean:male + I(age_demean^2):male +
      coordinate:male + holidays:male + hospital_per_area:male + PB_per_area:male + BM_per_area:male,
    data = .,
    se_type = "stata",
    linear_hypothesis = c(
...
names(rct$data)
with(rct$data, table(positive, exg_stop_test))
```
