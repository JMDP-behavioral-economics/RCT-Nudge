...
```{r lm-interaction-gender-test-donate, eval = FALSE}
rct$
...
```{r lm-interaction-coordination1-test-donate, eval = FALSE}
rct$
  lm_effect(c("test", "donate"), sample_drop = FALSE)$
...
```{r lm-interaction-gender-coordination-group-test-donate, eval = FALSE}
rct2 <- rct$clone()
rct2$data <- rct2$data %>%
  mutate(
    group = case_when(
...
```{r lm-who-selected-full, eval = FALSE}
test_dt <- rct$data %>%
  filter(test == 1) %>%
  mutate(
    age = age - mean(age),
...
```{r average-characteristics-among-CT, eval = FALSE}
rct$data %>%
  filter(test == 1) %>%
  datasummary(
    (`Male` = male) +
...
```{r relative-likelihood-complier-characteristics, eval = FALSE}
data_list <- list(
  full = rct$data,
