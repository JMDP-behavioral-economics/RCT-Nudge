...
rct_initial$set_default_se_type("stata")
...
est_skip_mods <- skip_mods %>%
  purrr::map(~ lm_robust(., data = skip_dt, se_type = "stata"))
...
```{r lm-interaction-gender-skip}
skip_interaction_mods <- list(
  coordinate_more ~ treat * male,
  coordinate_more ~ treat * male + age + I(age^2) + hospital_per_area +
    PB_per_area + BM_per_area + holidays + age:male + I(age^2):male + hospital_per_area:male + PB_per_area:male + BM_per_area:male + holidays:male,
  skip_CT ~ treat * male,
  skip_CT ~ treat * male + age + I(age^2) + hospital_per_area +
    PB_per_area + BM_per_area + holidays + age:male + I(age^2):male + hospital_per_area:male + PB_per_area:male + BM_per_area:male + holidays:male
)

hypothesis <- c(
  "treatB + treatB:male" = "Treatment B",
  "treatC + treatC:male" = "Treatment C",
  "treatD + treatD:male" = "Treatment D"
)

est_skip_interaction_mods <- skip_interaction_mods %>%
  map(~ lh_robust(
    .,
    data = skip_dt,
    se_type = "stata",
...
lh_tab <- est_skip_interaction_mods %>%
  map(~ .$lh) %>%
  modelsummary(
    coef_map = hypothesis,
    stars = c("***" = .01, "**" = .05, "*" = 0.1),
...
est_skip_interaction_mods %>%
  map(~ .$lm_robust) %>%
  modelsummary(
    title = "Heterogeneous Treatment Effects on Number of Coordination and Skipping CT by Gender",
    coef_map = c(
      "treatB" = "Treatment B",
      "treatC" = "Treatment C",
      "treatD" = "Treatment D",
  ...
  kableExtra::group_rows(
    "Linear combination test: Treatment + Treatment $\\times$ Male",
    15, 20,
    escape = FALSE
  ) %>%
  kableExtra::footnote(
...
plotdt %>%
  ggplot(aes(x = median_day, y = mean)) +
    ...
    my_theme_classic(size = 30) +
    theme(legend.position = "bottom")
```

```{r speed-CT-cond-response, fig.cap = "Binned Scatter Plot of CT vs. Response Spped among Those Who Responded with Positive Intention. \\newline \\emph{Note}: We used those who responded with positive intention to donate in the control arm (experimental arm A). The dashed line represents nonparametric fitting. The solid line represents a fitted line of the logistic regression.", fig.pos = "H"}
...
rct_noskip$
  flow(outcome = "positive", sample_drop = FALSE)$
  lm_time_segment(cut_days = c(5, 12))$
  fit(scale = 100)$
  kable_reg(
