...
第二のメッセージは、適合通知への早期返信を通じてコーディネーション期間を短くするために、早いコーディネーションが患者の移植到達率を高めるという情報を提示した。この情報は患者への移植によって得られる効用が時間を通じて減衰していくことを示している。いつでも患者を助けられると誤って信じているドナーを考える。時間に関する限界効用の低下が限界費用の低下を下回るので、そのドナーの最適な返信までの期間は長くなる。この情報はそうしたドナーの限界便益の低下の程度を修正することで、最適な返信までの期間を短くなると期待できる。
