...
#' subtitle: Coordination Process (Donation)
#' ---
#'
...
use <- rawdt %>%
  ...
  dplyr::filter(exg_stop_donate == 0)

#+ rcf-donate-run, include = FALSE
...
#+ rcf-donate-dist, fig.width = 10, fig.cap = "Heterogenous Message Effects on Donation."
...
ggplot(df, aes(x = effect, y = ..count.. / sum(..count..))) +
  ...
  geom_segment(
    aes(x = estimate, xend = estimate, y = 0, yend = 0.07),
    linetype = 2, data = ate
  ) +
  geom_text(
    aes(x = estimate, y = 0.072, label = summary),
    size = 4, data = ate
  ) +
  scale_y_continuous(
    expand = c(0, 0),
...
#+ rcf-donate-male-dist, fig.width=10, fig.height=10, fig.cap="Heterogenous Message Effects on Donation among Males."
...
#+ rcf-donation-female-dist, fig.width=10, fig.height=10, fig.cap = "Heterogenous Message Effects on Donation among Females."
