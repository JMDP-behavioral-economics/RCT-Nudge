...
#' subtitle: Coordination Process (Candidate)
#' ---
#'
...
use <- rawdt %>%
  ...
  dplyr::filter(exg_stop_candidate == 0)

#+ rcf-candidate-run, include = FALSE
...
#+ rcf-candidate-data, include = FALSE
age_group <- list(
  "Age: 20-29" = X[, 2] < 30,
...
#+ rcf-candiate-dist, fig.width = 10, fig.cap = "Heterogenous Message Effects on Candidate."
...
ggplot(df, aes(x = effect, y = ..count.. / sum(..count..))) +
  ...
  geom_text(
    aes(x = estimate, y = 0.067, label = summary),
    size = 4, data = ate
  ) +
  scale_y_continuous(
    expand = c(0, 0),
...
#+ rcf-candidate-male-dist, fig.width=10, fig.height=10, fig.cap="Heterogenous Message Effects on Candidate among Males."
...
#+ rcf-candidate-female-dist, fig.width=10, fig.height=10, fig.cap = "Heterogenous Message Effects on Candidate among Females."
...
#+ rcf-candidate-female-character
df %>%
  dplyr::filter(male == 0) %>%
  mutate(positive = if_else(effect > 0, "Positive", "Negative")) %>%
  datasummary(
