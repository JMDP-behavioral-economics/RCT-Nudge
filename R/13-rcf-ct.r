...
#' subtitle: Coordination Process (Confirmatory Typing)
#' ---
#'
...
use <- rawdt %>%
  ...
  dplyr::filter(exg_stop_test == 0)

#+ rcf-ct-run, include = FALSE
...
#+ rcf-ct-dist, fig.width = 10, fig.cap = "Heterogenous Message Effects on Confirmatory Typing."
...
ggplot(df, aes(x = effect, y = ..count.. / sum(..count..))) +
  ...
  geom_segment(
    aes(x = estimate, xend = estimate, y = 0, yend = 0.045),
    linetype = 2, data = ate
  ) +
  geom_text(
    aes(x = estimate, y = 0.047, label = summary),
    size = 4, data = ate
  ) +
  scale_y_continuous(
    expand = c(0, 0),
...
#+ rcf-ct-male-dist, fig.width=10, fig.height=10, fig.cap="Heterogenous Message Effects on Confirmatory Typing among Males."
...
df %>%
  ...
  geom_text(
    aes(x = estimate, y = 0.035, label = summary),
...
#+ rcf-ct-female-dist, fig.width=10, fig.height=10, fig.cap = "Heterogenous Message Effects on Confirmatory Typing among Females."
