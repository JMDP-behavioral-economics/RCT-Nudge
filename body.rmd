...
はじめに、統計的推論に対する頑健性を検証する。ここまでの結果はトリートメントの割り当てに基づいて週レベルでクラスターした標準誤差を用いている。しかしながら、Camere (2008)はクラスターの数が少ないと、帰無仮説を過度に棄却してしまう恐れがあることを指摘し、クラスターワイルドブートストラップ法によるp値の計算を推奨している。我々は図\@ref(fig:subsample-reply)の結果にこの手法を適用して、統計的な有意性が頑健であるかどうかを検証する（表\@ref(tab:robust-subsample-reply)）。その結果、統計的な有意性は下がるものの、確率メッセージ（介入群B）が30歳未満の男性の正の意向を伴う返信率を統計的に10%水準で有意に高めている。
...
```{r}
#| label: rcf-intention
#| fig.cap: "Distribution of individual treatment effects on the positive intention estimated by the random causal effect."
tau <- predict(rcf, X)
plotdt <- data.frame(X) %>%
...
次に、高次元の共変量の組み合わせによる効果の異質性を検証するために、機械学習の一種であるcausal forestを用いる。Causal forestは共変量の組み合わせでサブサンプルを作成し、各サブサンプル内で効果を推定する（これを個人レベルの介入効果とする）手法である。サブサンプルを作成するとき、この手法は予測効果の平均二乗誤差を最小にすることを通じて、効果の異質性を最大にするようにする。
...
add_table <- data.frame(rbind(
  ...
  c("B = 0", sprintf("%1.2f", wildBS_p$treatB)),
  c("D = 0", sprintf("%1.2f", wildBS_p$treatD))
))
