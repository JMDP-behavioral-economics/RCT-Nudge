...
```{r reg-stock-subset, include=FALSE}
est_stock_sub <- stock %>%
...
ctrl <- with(subset(use, treat == "A" & reply == 1), mean(days_reply))
ctrl_positive <- with(subset(use, treat == "A" & reply == 1 & intention == 1), mean(days_reply))
ctrl_negative <- with(subset(use, treat == "A" & reply == 1 & intention == 0), mean(days_reply))
ctrl_max <- with(subset(use, treat == "A" & reply == 1), max(days_reply))
ctrl_7days <- with(subset(flow, treat == "A" & within == 7 & outcome == "Reply"), mean(value))
ctrl_positive_7days <- with(subset(flow, treat == "A" & within == 7 & outcome == "Positive intention"), mean(value))
```
...
plot_list_young_male <- unique(est_flow_sub$outcome) %>%
  purrr::map(function(x) {
    subset(est_flow_sub, male == 1 & age_less30 == 1 & outcome == x) %>%
      ...
      scale_x_continuous(
        breaks = subset(est_flow_sub, male == 1 & age_less30 == 1 & outcome == x)$within,
        labels = subset(est_flow_sub, male == 1 & age_less30 == 1 & outcome == x)$avg
      ) +
      scale_y_continuous(
        breaks = seq(-0.2, 0.2, by = 0.05), limits = c(-0.2, 0.2)
      ) +
...
wrap_plots(plot_list_young_male, ncol = 2) +
  plot_layout(guides = "collect") &
  theme(legend.position = "bottom")
```
...
plot_list_young_female <- unique(est_flow_sub$outcome) %>%
  purrr::map(function(x) {
    subset(est_flow_sub, male == 0 & age_less30 == 1 & outcome == x) %>%
      ggplot(aes(x = within, y = estimate, color = term, shape = term)) +
      geom_hline(aes(yintercept = 0), linetype = 2) +
      geom_point(size = 3, position = position_dodge(2)) +
      geom_errorbar(
      ...
      scale_x_continuous(
        breaks = subset(est_flow_sub, male == 0 & age_less30 == 1 & outcome == x)$within,
        labels = subset(est_flow_sub, male == 0 & age_less30 == 1 & outcome == x)$avg
      ) +
      scale_y_continuous(
...
wrap_plots(plot_list_young_female, ncol = 2) +
  plot_layout(guides = "collect") &
  theme(legend.position = "bottom")
```
...
```{r coordination-data, include=FALSE}
exclude <- use %>%
...
```{r reg-subsample-coordination, include=FALSE}
...
```{r wildbs-young-male-stock, eval = FALSE, tab.cap = 'Linear Probability Model of Reply and Intentions for Males Less Then 30 Years'}
...
```{r wildbs-young-male-coordination, eval = FALSE, tbl.cap = 'Linear Probability Model of Coordination among Males'}
...
plot_list_old_male <- unique(est_flow_sub$outcome) %>%
  purrr::map(function(x) {
    subset(est_flow_sub, male == 1 & age_less30 == 0 & outcome == x) %>%
      ggplot(aes(x = within, y = estimate, color = term, shape = term)) +
      geom_hline(aes(yintercept = 0), linetype = 2) +
      geom_point(size = 3, position = position_dodge(2)) +
      geom_errorbar(
      ...
      scale_x_continuous(
        breaks = subset(est_flow_sub, male == 1 & age_less30 == 0 & outcome == x)$within,
        labels = subset(est_flow_sub, male == 1 & age_less30 == 0 & outcome == x)$avg
      ) +
      scale_y_continuous(
        breaks = seq(-0.2, 0.2, by = 0.05), limits = c(-0.2, 0.2)
      ) +
...
wrap_plots(plot_list_old_male, ncol = 2) +
  plot_layout(guides = "collect") &
  theme(legend.position = "bottom")
```
...
plot_list_old_female <- unique(est_flow_sub$outcome) %>%
  purrr::map(function(x) {
    subset(est_flow_sub, male == 0 & age_less30 == 0 & outcome == x) %>%
      ggplot(aes(x = within, y = estimate, color = term, shape = term)) +
      geom_hline(aes(yintercept = 0), linetype = 2) +
      geom_point(size = 3, position = position_dodge(2)) +
      geom_errorbar(
      ...
      scale_x_continuous(
        breaks = subset(est_flow_sub, male == 0 & age_less30 == 0 & outcome == x)$within,
        labels = subset(est_flow_sub, male == 0 & age_less30 == 0 & outcome == x)$avg
      ) +
      scale_y_continuous(
        breaks = seq(-0.2, 0.2, by = 0.05), limits = c(-0.2, 0.2)
      ) +
...
wrap_plots(plot_list_old_female, ncol = 2) +
  plot_layout(guides = "collect") &
  theme(legend.position = "bottom")
```
