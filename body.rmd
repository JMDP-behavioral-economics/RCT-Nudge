...
schedule <- read_csv(here(data_root, "RCT-schedule.csv")) %>%
  mutate(
...
summary_experiment(use$cs, FALSE, "HC1") %>%
  tbl()
```
...
lm_all_stock(use$stock, FALSE, "HC1", TRUE) %>%
  tbl()
```
...
lm_subset_stock(use$stock, FALSE, "HC1", TRUE) %>%
  coefplot()
```
...
Y <- use$cs$intention
X <- model.matrix(covariate, data = use$cs)
W <- use$cs$treat
G <- use$cs$RCTweek
...
est_coordination <- use$coordination %>%
  group_by(outcome) %>%
  nest() %>%
  mutate(
    fit1 = map(
...
est_coordination_sub <- use$coordination %>%
  mutate(age_less30 = if_else(age < 30, 1, 0)) %>%
...
est_coordination_logit <- use$coordination %>%
  group_by(outcome) %>%
  nest() %>%
  mutate(
    fit1 = map(
...
ate_bs_coordination <- use$coordination %>%
  dplyr::filter(male == 1) %>%
