...
We received the coordination data at the end of June 2022 and verified the effects of the messages. Our main outcomes are the response to the compatibility notice, which strongly reflects the donor's willingness, and the reaching the confirmatory typing, which represents donor's availability. Regarding the response to the compatibility notice, the data records the donor's willingness to donate and the time it takes to respond. First, we estimate the overall average effect of the two messages. Even if the overall average effect is positive, the effect on specific gender and age groups may be negative. In such a case, a policy that assigns the message with a positive effect only to specific groups would be preferable to a uniform policy of assigning it to all. Moreover, interventions that increase the willingness of young male donors and prevent their attrition may differ from interventions with an overall positive average effect. These possibilities motivate us to examine the heterogeneity of the message effects.
...
We show sample averages of each outcome by experimental arms in Figure \ref{fig:stock-diff-mean}. Although statistical significance is marginal, experimental arm B, which includes the probability message, may increase response rates and positive intentions by \(2.3\) percentage points, suggesting that the probability message enhances donor willingness. We show the estimation results of linear probability model in Table \ref{tab:stock-reg}. When controlling for covariates, although statistical siginificance is marginal, experimental arm B may also increase response rates. However, logistic regressions show that 95\% confidence intervals for odds ratios include 1 and fails to reject the null hypothesis that there is no message effect (see Table \ref{tab:stock-logit} in the Appendix \ref{figtab}). Therefore, we cannot robustly obtain the result that the message improves donor intentions.
...
Table \ref{tab:stock-reg-subset-2} in the Appendix \ref{figtab} presents the subsample analysis with an age cutoff set at 40. The effect of the probability message (experimental arm B) on young males slightly weakens, while it slightly strengthens for older males. Particularly, the effect on the response rate of older males is statistically significant. This suggests that in males, the effect of the probability message follows a convex function of age. However, there is no change in our overall conclusion.
...
We show sample averages of each outcome by treatments in Figure \ref{fig:coordinate-diff-mean}. No experimental arm affects the post-candidate selection process. Similar results are obtained with linear probability models and logistic regressions controlling for covariates (see Table \ref{tab:coordinate-reg} and \ref{tab:coordinate-logit} in Appendix \ref{figtab}). This result should be interpreted with caution. Given that our intervention does not affect demand (number of patients), if our intervention increases the number of people who reached the CT, it should increase the number of people who are not selected as candidates for exogenous reasons. Particularly, compared to the control arm, experimental arm B increases the probability by \(3.5\) percentage points of donors who reached CT not being selected due to patient reasons or donor health reasons. This difference is statistically significant. Thus, intervention effects are canceled out due to demand-side factor.
