...
```{r geographical-hetero-secondaries, include = FALSE}
est <- testdt %>%
...
```{r plotB-geographical-hetero-secondaries, fig.width = 10, fig.height=7}
est %>%
  dplyr::filter(treat == "B") %>%
...
```{r plotC-geographical-hetero-secondaries, fig.width = 10, fig.height=7}
est %>%
  dplyr::filter(treat == "C") %>%
  ggplot(aes(x = qt_hospital, y = estimate)) +
    geom_hline(aes(yintercept = 0), linetype = 2) +
...
```{r plotD-geographical-hetero-secondaries, fig.width = 10, fig.height=7}
est %>%
  dplyr::filter(treat == "D") %>%
  ggplot(aes(x = qt_hospital, y = estimate)) +
    geom_hline(aes(yintercept = 0), linetype = 2) +
