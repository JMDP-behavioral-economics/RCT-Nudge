...
geohetero_reply <- testdt %>%
  mutate(hospital = if_else(
    prefecture %in% c("東京都", "大阪府", "神奈川県", "愛知県"),
    1, 0
  )) %>%
  group_by(outcome, within, hospital) %>%
  do(est = lm_robust(
    update(mod, . ~ . - prefecture),
    cluster = RCTweek,
    se_type = "stata",
    data = .
  )) %>%
...
geohetero_reply %>%
  ...
  ggplot(aes(x = hospital, y = estimate, color = outcome, shape = outcome)) +
    geom_hline(aes(yintercept = 0), linetype = 2) +
    geom_pointrange(
      aes(ymin = conf.low, ymax = conf.high),
      position = position_dodge(0.5)
