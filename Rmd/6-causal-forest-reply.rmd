...
```{r causal-forest-reply-dist, fig.width = 10}
...
```{r causal-forest-reply-dist-gen, fig.width = 10, fig.height = 5}
average_tau_male <- average_treatment_effect(est, subset = X[, 1] == 1) %>%
  data.frame() %>%
  mutate(treat = str_extract(contrast, "[B-D]")) %>%
  mutate(treat = factor(treat, labels = paste("Message", treat))) %>%
  mutate(male = 1)

average_tau_female <- average_treatment_effect(est, subset = X[, 1] == 0) %>%
  data.frame() %>%
  mutate(treat = str_extract(contrast, "[B-D]")) %>%
  mutate(treat = factor(treat, labels = paste("Message", treat))) %>%
  mutate(male = 0)
...
cate_summary <- average_tau_male %>%
  bind_rows(average_tau_female) %>%
  left_join(negative_effect, by = c("treat", "male")) %>%
  mutate(summary = sprintf(
    "ATE: %1.3f (s.e. = %1.3f)\n%1.1f%% negative effect",
    estimate, std.err, negative * 100
  )) %>%
  mutate(male = factor(male, labels = c("Subset: Females", "Subset: Males")))
...
```{r cf-reply-gen-age, fig.width = 10, fig.height=10}
plotdt %>%
  mutate(male = factor(
    male, labels = c("Subset: Females", "Subset: Males")
  )) %>%
