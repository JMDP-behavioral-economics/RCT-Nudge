...
## Hospitals per 10 square kilometers -->

```{r plot-hospital-per-area, fig.width=10, fig.height=7, eval = FALSE}
hospital %>%
  ggplot(aes(
    x = reorder(prefecture, hospital_per_area), y = hospital_per_area
  )) +
  geom_hline(aes(yintercept = 0)) +
  geom_bar(aes(fill = qt_hospital), stat = "identity", color = "black") +
  scale_fill_grey(start = 0.7, end = 0.1) +
  labs(
    x = "Prefectures",
    y = "Hospitals per 10 square kilometers",
    fill = "Quartiles"
  ) +
...
```{r geographical-hetero-primaries, include = FALSE, eval = FALSE}
est <- testdt %>%
...
```{r plotB-geographical-hetero-primaries, fig.width = 10, fig.height=7, eval = FALSE}
est %>%
...
```{r plotC-geographical-hetero-primaries, fig.width = 10, fig.height=7, eval = FALSE}
est %>%
...
```{r plotD-geographical-hetero-primaries, fig.width = 10, fig.height=7, eval = FALSE}
est %>%
