...
```{r plot-cond-int}
tibble(time = 5:max(testdt$days_reply, na.rm = TRUE)) %>%
  rowwise(time) %>%
  summarize(
...
plotdt <- tibble(time = 5:max(testdt$days_reply, na.rm = TRUE)) %>%
  rowwise(time) %>%
  summarize(
    subset(testdt, reply == 1 & days_reply <= time) %>%
      group_by(treat, male, age_less30) %>%
      summarize(mean = mean(intention)) %>%
      ungroup()
  ) %>%
  ungroup()

subset(plotdt, male == 0 & age_less30 == 0) %>%
  ggplot(aes(x = time, y = mean, group = treat)) +
    geom_line(aes(color = treat)) +
    scale_y_continuous(breaks = seq(0, 1, 0.1)) +
    scale_x_continuous(breaks = seq(0, 90, 5)) +
...
subset(plotdt, male == 0 & age_less30 == 1) %>%
  ggplot(aes(x = time, y = mean, group = treat)) +
  geom_line(aes(color = treat)) +
  scale_y_continuous(breaks = seq(0, 1, 0.1)) +
  scale_x_continuous(breaks = seq(0, 90, 5)) +
...
subset(plotdt, male == 1 & age_less30 == 0) %>%
  ggplot(aes(x = time, y = mean, group = treat)) +
  geom_line(aes(color = treat)) +
  scale_y_continuous(breaks = seq(0, 1, 0.1)) +
  scale_x_continuous(breaks = seq(0, 90, 5)) +
...
subset(plotdt, male == 1 & age_less30 == 1) %>%
  ggplot(aes(x = time, y = mean, group = treat)) +
  geom_line(aes(color = treat)) +
  scale_y_continuous(breaks = seq(0, 1, 0.1)) +
  scale_x_continuous(breaks = seq(0, 90, 5)) +
