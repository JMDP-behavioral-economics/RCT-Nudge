...
```{r rcf-negative-dist, fig.width = 10, fig.cap="Heterogenous Message Effects on Negative Intention"}
ate <- average_treatment_effect(est) %>%
  data.frame() %>%
  mutate(treat = str_extract(contrast, "[B-D]")) %>%
  mutate(treat = factor(treat, labels = paste("Message", treat))) %>%
...
plotdt %>%
  ...
  geom_segment(
    aes(x = estimate, xend = estimate, y = 0, yend = 0.035),
    linetype = 2, data = ate
  ) +
  geom_text(
    aes(x = estimate, y = 0.038, label = summary),
    size = 4, data = ate
  ) +
  scale_y_continuous(
    expand = c(0, 0),
...
```{r rcf-negative-male-dist, fig.width = 10, fig.height=10, fig.cap = "Heterogeneous Message Effects on Negative Intention among Males."}
male <- X[, 1] == 1
...
plotdt %>%
  ...
  geom_text(
    aes(x = estimate, y = 0.023, label = summary),
    size = 4, data = ate_male
  ) +
  scale_fill_manual(values = c("white", "red")) +
  scale_y_continuous(
...
```{r rcf-negative-female-dist, fig.width = 10, fig.height = 10, fig.cap = "Heterogeneous Message Effects on Negative Intention among Females."}
female <- X[, 1] == 0
